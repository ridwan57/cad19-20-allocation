...
import { Button, Tooltip, Select, MenuItem, FormControl, InputLabel, TableFooter, TablePagination, TableRow, IconButton, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, } from '@material-ui/core';
...
import { boardModuleRootAddress, rankingTypeCodes, rankingStatusCodes, rankingTypesAvailable, LROCodes, precisionFields, rankingCategory, rankedApplicantCountKey, rankingTypeTextToCode } from '../../data/constants';
...
class RankingLanding extends Component {
    ...
    handleResize = (e) => {
        if (this.props.state === 'landing') this.setState({ tableHeight: window.innerHeight - 2 * this.state.divElement.getBoundingClientRect().bottom });
    }
    componentDidMount() {
        ...
        this.setState({
            tableHeight: window.innerHeight - 350 - this.divElement.getBoundingClientRect().bottom - 20,
        ...
        fetch(boardModuleRootAddress + '/getRankingMetaData/' + LROCodes[this.props.category].scienceTypeRankingCode, {
            headers: {
            ...
            .then(dataForScience => {
                ...
                fetch(boardModuleRootAddress + '/getRankingMetaData/' + LROCodes[this.props.category].allTypeRankingCode, {
                    headers: {
                        'Authorization': `bearer ${this.props.keycloak.token}`
                    }
                })
                    ...
                    .then(dataForAll => {
                        ...
                        this.IntervalScience = setInterval(async () => {
                            this.setState({ currentTime: Date.now() });
                            if (this.state.scienceRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusRunning) {
                                fetch(boardModuleRootAddress + '/getRankingMetaData/' + LROCodes[this.props.category].scienceTypeRankingCode, {
                                    headers: {
                                        'Authorization': `bearer ${this.props.keycloak.token}`
                                    }
                        ...
                        this.IntervalAll = setInterval(async () => {
                            this.setState({ currentTime: Date.now() });
                            if (this.state.allRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusRunning) {
                                fetch(boardModuleRootAddress + '/getRankingMetaData/' + LROCodes[this.props.category].allTypeRankingCode, {
                                    headers: {
                                        'Authorization': `bearer ${this.props.keycloak.token}`
                                    }
    ...
    changeRankingType(event) {
        ...
        if (event.target.value < 0 || (event.target.value === rankingTypeCodes.allTypeRankingCode && this.state.allRankingMetaData.rankingStatus === null) || ((event.target.value === rankingTypeCodes.scienceTypeRankingCode && this.state.scienceRankingMetaData.rankingStatus === null))) {
            console.log("test");
            this.setState({
                fetchRankedListDisable: true,
                rankedList: []
            ...
            fetch(boardModuleRootAddress + '/getRankingCriteria/' + event.target.value, {
                headers: {
                    'Authorization': `bearer ${this.props.keycloak.token}`
                }
            })
                ...
                .then(data => {
                    if (data.result) {
                        let rankingCriteria = [];
                        if (event.target.value === rankingTypeCodes.scienceTypeRankingCode) {
                        ...
                        let rowCount = 0;
                        fetch(boardModuleRootAddress + rankingCategory[this.props.category].countAPI, {
                            headers: {
                                'Authorization': `bearer ${this.props.keycloak.token}`
                            }
                        }).then(response => response.json()).then(data => {
                            const key = rankingCategory[this.props.category].countKeyJSON + rankedApplicantCountKey[this.state.rankingTypeCodeForRankedList].keyJSON;
                            console.log(key);
    ...
    startRanking() {
        let opCode = 0;
        if (rankingTypeTextToCode[this.props.rankingType] === rankingTypeCodes.scienceTypeRankingCode) opCode = LROCodes[this.props.category].scienceTypeRankingCode;
                ...
                >
                    ...
                    <DialogContent>
                        <DialogContentText id="alert-dialog-description" style={{ textAlign: 'center' }}>
                            {this.props.rankingType} type ranking  has been successfully started...
                        </DialogContentText>
                    </DialogContent>
                </Dialog>
                ...
                <div className="flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4" >
                    {
                        <div className="d-block mb-4 mb-xl-0">
                            <Breadcrumb className="d-none d-md-inline-block" listProps={{ className: "breadcrumb-dark breadcrumb-transparent" }}>
                                ...
                                <Breadcrumb.Item active>{rankingCategory[this.props.category].text}</Breadcrumb.Item>
                            </Breadcrumb>

                            <div class="col d-flex justify-content-center ranking" ref={(divElement) => { this.divElement = divElement }}>
                                <Card style={{ marginRight: "2%" }} >
                                    <Card.Header>
                                        <div className="row">
                                            <div className="col-10">
                                                Science Type Ranking
                                            </div>
                                    ...
                                    <Card.Body>
                                            ...
                                            <div className="col-6">
                                                {
                                                    (
                                                        ...
                                                        this.state.scienceRankingMetaData.rankingCriteriaLastUpdateDate !== null &&
                                                        moment(this.state.scienceRankingMetaData.rankingCriteriaLastUpdateDate).add(6, 'hours').format("DD.MM.YYYY, h:mm:ss A")
                                                    )
                                                    ||
                                                    (
                                                        this.state.metaDataUpdated &&
                                            ...
                                            <div className="col-6 gray-text">
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        (!this.state.scienceRankingMetaData.rankingStatus || this.state.scienceRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusCompleted) &&
                                                        'Ranking Updated On'
                                                    )
                                                }
                                                {
                                            ...
                                            <div className="col-6">
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        this.state.scienceRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusCompleted &&
                                                        this.state.scienceRankingMetaData.rankingLastUpdateDate !== null &&
                                                        moment(this.state.scienceRankingMetaData.rankingLastUpdateDate).format("DD.MM.YYYY, h:mm:ss A")
                                                    )
                                                    ||
                                                    (
                                                        this.state.metaDataUpdated &&
                                                ...
                                                }
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        this.state.scienceRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusRunning &&
                                                        moment(this.state.scienceRankingMetaData.rankingProcessStartDateTime).format("DD.MM.YYYY, h:mm:ss A")
                                                    )
                                                }
                                            </div>
                                        </div>
                                        ...
                                        <div className="row">
                                            <div className="col-7 gray-text">
                                                {
                                                    this.state.metaDataUpdated && this.state.scienceRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusRunning &&
                                                            ...
                                                            <div className="col-6">
                                                                ...
                                                                <Row>
                                                                    <div className="col-12 bold-text">
                                                                        {moment(moment(this.state.currentTime) - moment(this.state.scienceRankingMetaData.rankingProcessStartDateTime)).subtract(6, 'hours').format("HH:mm:ss")}
                                                                    </div>
                                                                </Row>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="col-5 ranking-button">
                                                <div className="button">
                                                    ...
                                                    <Button
                                                        onClick={() => this.props.updateStateFunction('criteria-view', 'Science')}
                                                        variant="contained"
                                                        color="primary"
                                                        size="medium"
                                                        disabled={false}
                                ...
                                <Card>
                                    ...
                                    <Card.Body>
                                            ...
                                            <div className="col-6">
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        this.state.allRankingMetaData.rankingCriteriaLastUpdateDate !== null &&
                                                        moment(this.state.allRankingMetaData.rankingCriteriaLastUpdateDate).add(6, 'hours').format("DD.MM.YYYY, h:mm:ss A")
                                                    )
                                                    ||
                                                    (
                                                        this.state.metaDataUpdated &&
                                            ...
                                            <div className="col-6 gray-text">
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        (!this.state.allRankingMetaData.rankingStatus || this.state.allRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusCompleted) &&
                                                        'Ranking Updated On'
                                                    )
                                                }
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        this.state.allRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusRunning &&
                                                        'Ranking Started On'
                                                    )
                                                }
                                            </div>
                                            <div className="col-6">
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        this.state.allRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusCompleted &&
                                                        this.state.allRankingMetaData.rankingLastUpdateDate !== null &&
                                                        moment(this.state.allRankingMetaData.rankingLastUpdateDate).format("DD.MM.YYYY, h:mm:ss A")
                                                    )
                                                    ||
                                                    (
                                                        this.state.metaDataUpdated &&
                                                ...
                                                }
                                                {
                                                    (
                                                        this.state.metaDataUpdated &&
                                                        this.state.allRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusRunning &&
                                                        moment(this.state.allRankingMetaData.rankingProcessStartDateTime).format("DD.MM.YYYY, h:mm:ss A")
                                                    )
                                                }
                                            </div>
                                        </div>
                                        ...
                                        <div className="row">
                                            <div className="col-7 gray-text">
                                                {
                                                    this.state.metaDataUpdated && this.state.allRankingMetaData.rankingStatus === rankingStatusCodes.rankingStatusRunning &&
                                                    <div className="process-time">
                                                        <div className="row">
                                                            ...
                                                            <div className="col-6">
                                                                ...
                                                                <Row>
                                                                    <div className="col-12 bold-text">
                                                                        {moment(moment(this.state.currentTime) - moment(this.state.allRankingMetaData.rankingProcessStartDateTime)).subtract(6, 'hours').format("HH:mm:ss")}
                                                                    </div>
                                                                </Row>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="col-5 ranking-button">
                                                <div className="button">
                                                    <Button
                                                        onClick={() => this.props.updateStateFunction('criteria-view', 'All')}
                                                        variant="contained"
                                                        color="primary"
                                                        size="medium"
                                                        disabled={false}
                            ...
                            <div class="col d-flex justify-content-center ranking" style={{ paddingTop: "20px" }}>
                                                ...
                                                <div className="col-5">
                                                    <div className="row">
                                                        <div className="col-12 header">
                                                            Ranked {rankingCategory[this.props.category].fetched}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="col-7">
                                        ...
                                        <Accordion.Body>
                                            <div className='row'>
                                                <div className='col-4' style={{ paddingTop: '15px' }}>
                                                    <FormControl variant="">
                                                        <InputLabel id="demo-simple-select-label">Ranking Type</InputLabel>
                                                        <Select
                                                        ...
                                                        >
                                                            ...
                                                            {this.state.rankingTypesAvailable.map(element => <MenuItem value={element.typeCode}>{element.typeText}</MenuItem>)}
                                                        </Select>
                                                    </FormControl>
                                                </div>

                                                <div className="col-8 button" style={{ textAlign: 'left', verticalAlign: 'middle' }}>
                                                    <Button
                                                    ...
                                                    //startIcon={<SaveIcon />}
                                                    >
                                                        View Ranked {rankingCategory[this.props.category].fetched}
                                                    </Button>
                                                </div>
                                            </div>
