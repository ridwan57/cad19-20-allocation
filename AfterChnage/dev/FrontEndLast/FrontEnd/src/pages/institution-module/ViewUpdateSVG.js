...
class ViewUpdateSVG extends Component {
  constructor (props) {
    ...
    this.handleCancelUpdateButtonPressed = this.handleCancelUpdateButtonPressed.bind(
      this
    )
    this.handleConfirmUpdateButtonPressed = this.handleConfirmUpdateButtonPressed.bind(
      this
    )
    this.PaperComponent = this.PaperComponent.bind(this)
    this.handleUpdatedMinGPAChange = this.handleUpdatedMinGPAChange.bind(this)
    this.handleUpdatedOwnMinGPAChange = this.handleUpdatedOwnMinGPAChange.bind(
      this
    )
    this.handleUpdatedSQMinGPAChange = this.handleUpdatedSQMinGPAChange.bind(
      this
    )
    this.handleUpdateSuccessAcknowledged = this.handleUpdateSuccessAcknowledged.bind(
      this
    )
  }
      ...
      .then(data => {
        ...
        if (data.status === 200) {
          ...
          lst.forEach((element, index) => {
            ...
            element.minimumGPA = (
              Math.round(element.minimumGPA * 100) / 100
            ).toFixed(2)
            element.ownMinimumGPA = (
              Math.round(element.ownMinimumGPA * 100) / 100
            ).toFixed(2)
            element.sqminGPA = (
              Math.round(element.sqminGPA * 100) / 100
            ).toFixed(2)
            element.gender =
              (element.gender === 'F' && 'Female') ||
              (element.gender === 'M' && 'Male') ||
              (element.gender === 'C' && 'Co-Ed.')
            if (element.isSQ === 'N') {
              element.sqminGPA = 'N/A'
              element.isSQ = 'No'
            } else element.isSQ = 'Yes'
            element.action = (
              <Tooltip title='Update'>
                <IconButton
                  ...
                  onClick={() => this.handleEditSVGButtonPressed(index)}
                >
                  <EditIcon />
                </IconButton>
              </Tooltip>
  ...
  handleConfirmUpdateButtonPressed (event) {
    if (
      this.state.updatedMinGPA === '' &&
      this.state.updatedOwnMinGPA === '' &&
      this.state.updatedSQMinGPA === ''
    ) {
    ...
    updatedSVGToSubmit.minGPA = (
      Math.round(this.state.updatedMinGPA * 100) / 100
    ).toFixed(2)
    updatedSVGToSubmit.ownMinGPA = (
      Math.round(this.state.updatedOwnMinGPA * 100) / 100
    ).toFixed(2)
    updatedSVGToSubmit.sqMinGPA = (
      Math.round(this.state.updatedSQMinGPA * 100) / 100
    ).toFixed(2)
      ...
      <>
        ...
        <div>
          ...
          >
            ...
            <DialogActions>
              <Button
                autoFocus
                color='primary'
                onClick={this.handleCancelUpdateButtonPressed}
              >
                Cancel
              </Button>
              <Button
                onClick={this.handleConfirmUpdateButtonPressed}
          ...
          >
            ...
            <DialogActions>
              <Button
                onClick={this.handleUpdateSuccessAcknowledged}
                color='primary'
                autoFocus
              >
                OK
