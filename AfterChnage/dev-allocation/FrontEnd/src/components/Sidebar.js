				...
				<SimpleBar {...events} className={`${contracted} collapse ${showClass} sidebar d-md-block bg-primary text-white`}>
					<div className="sidebar-inner px-4 py-3 overflow-hidden">
						...
						<Nav className="flex-column pt-3 pt-md-0">
							<NavItem styl={{ paddingTop: '70px' }} link={'/'} title="IEIMS" image={ReactHero} />
							<br />
							<br />

							{sideBarOptions[props.keycloak.realmRole].map(element =>
								(
									element.type === 'single' && 
                  <NavItem key={element.title} title={element.title} link={element.link} multiLinks={element.multiLinks} icon={element.icon} />
								) 
                  ||
                (
                	element.type === 'collapsable' && 
                  <CollapsableNavItem key={element.eventKey} eventKey={element.eventKey} title={element.title} icon={element.icon}>
                  	{ props.applicant &&
                      element.items[props.applicant.applicationStatus].map(subElement =>
                      	<NavItem title={subElement.title} link={subElement.link} />
                      )
                  	}
                  	{
                  		!props.applicant &&
                      element.items.map(subElement =>
                      	<NavItem key={subElement.title} title={subElement.title} link={subElement.link} />
                      )
                  	}
                  </CollapsableNavItem>
                ) 
