...
class AllocatedAapplicants extends Component {
        ...
        if (data.result.length > 0) {
          ...
          data.result.forEach((element, index) => {
            ...
            //   (element.applicationStatus === 3 && 'Cancelled')
              (element.applicationStatus === CADApplicationStatuses.AllocationConfirmed&& 'Allocated') ||
              (element.applicationStatus === CADApplicationStatuses.Admitted && 'Admitted') ||
              (element.applicationStatus === CADApplicationStatuses.Cancelled && 'Cancelled')
              element.action = (
              <div>
  ...
  handleConfirmAdmissionButtonPressed (index) {
    ...
    updatedAllocatedApplicants[index].applicationStatus = CADApplicationStatuses.Admitted
  ...
  handleCancelAdmissionButtonPressed (index) {
    ...
    updatedAllocatedApplicants[index].applicationStatus = CADApplicationStatuses.Cancelled
      ...
      .then(data => {
        if (data.message === 'Success') {
          selectedRows.data.forEach(element => {
            ...
            updatedAllocatedApplicants[element.index].applicationStatus = CADApplicationStatuses.Admitted
          })
          this.setState({ allocatedApplicants: updatedAllocatedApplicants })
      ...
      .then(data => {
        if (data.message === 'Success') {
          selectedRows.data.forEach(element => {
            ...
            updatedAllocatedApplicants[element.index].applicationStatus = CADApplicationStatuses.Cancelled
          })
          this.setState({ allocatedApplicants: updatedAllocatedApplicants })
          selectedRows.data.splice(0, selectedRows.data.length)
          selectedRows.lookup = {}
