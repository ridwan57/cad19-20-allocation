...
class UploadAdmissionData extends Component {
            ...
            .then(data => {
                if (data.result.length > 0) {
                    ...
                    lst.forEach((element, index) => {
                        element.action = <div>
                            <Tooltip title="Edit">
                                <IconButton aria-label="add" color="primary" style={{ marginTop: "-4px", width: "32px", height: "32px", minHeight: "32px" }} onClick={() => this.handleEditButtonPressed(index)}>
                                    <EditIcon />
                                </IconButton>
                            </Tooltip>
                            <Tooltip title="Delete">
                                <IconButton aria-label="add" color="secondary" style={{ marginTop: "-4px", width: "32px", height: "32px", minHeight: "32px" }} onClick={() => this.handleDeleteButtonPressed(index)}>
                                    <DeleteIcon />
                                </IconButton>
                            </Tooltip>
                        </div>
    ...
    handleShiftSelection(e) {
        ...
        this.state.svgs.forEach((element, index) => {
            if (element.shift.shiftName === e.target.value) versions.push(element.version.versionName);
        });
        this.setState({ shiftName: e.target.value, selectedShift: e.target.value, vsersion: '', hscGroupName: '', versions: [...new Set(versions)], groups: [] })
    }

    handleVersionSelection(e) {
        let groups = []
        this.state.svgs.forEach((element, index) => {
            if (element.shift.shiftName === this.state.selectedShift && element.version.versionName === e.target.value) groups.push(element.hscGroup.hscGroupName);
        });
        this.setState({ versionName: e.target.value, selectedVersion: e.target.value, hscGroupName: '', groups: [...new Set(groups)] });
    }

    findSVGID(shift, version, group) {
        const svgIndex = this.state.svgs.findIndex(function (svg) {
            return svg.shift.shiftName === shift && svg.version.versionName === version && svg.hscGroup.hscGroupName === group
        })
    ...
    addAdmissionData() {
        const newSingleAdmissionData = {
            ...
            "esvgId": this.findSVGID(this.state.shiftName, this.state.versionName, this.state.hscGroupName),
        }

        const newAdmissionData = [newSingleAdmissionData].concat(this.state.admissionData);
        ...
        const admissionDataToSave = {
            CRVSID: this.state.CRVSID,
            quotaFF: "No",
            allocatedESVG: this.findSVGID(this.state.shiftName, this.state.versionName, this.state.hscGroupName),
        }
        ...
        fetch(institutionModuleRootAddress+'/addAdmissionData/', {
            ...
            .then(data => {
                ...
                if (data.message === 'Success') {
                    this.setState({ admissionData: newAdmissionData, showAddAdmissionDataWindow: false, CRVSID: '', applicantName: '', shiftName: '', versionName: '', hscGroupName: '' });
                }
            })
    }
    ...
    handleEditButtonPressed(index) {
        ...
        let versions = []
        this.state.svgs.forEach((element, index) => {
            if (element.shift.shiftName === currentShift) versions.push(element.version.versionName);
        });
        ...
        let groups = []
        this.state.svgs.forEach((element, index) => {
            if (element.shift.shiftName === currentShift && currentVersion === element.version.versionName) groups.push(element.hscGroup.hscGroupName);
        });
    ...
    updateAdmissionData() {
        ...
        fetch(institutionModuleRootAddress+'/addAdmissionData/', {
            ...
            .then(data => {
                console.log(data);
                if (data.message === 'Success') {
                    this.setState({ admissionData: newAdmissionData, showEditWindow: false, CRVSID: '', applicantName: '', shiftName: '', versionName: '', hscGroupName: '' });
                }
            })
    ...
    fetchApplicantName() {
        ...
        fetch(institutionModuleRootAddress+'/getCRVSProfile/' + this.state.CRVSID, {
            headers: {
                'Authorization': `Bearer ${this.props.keycloak.token}`,
            },
            ...
            <>
                ...
                >
                            ...
                            <div class="row upload-admission-data-form">
                                ...
                                <div class="col-sm-12 col-md-4">
                                    <FormControl>
                                        ...
                                        >
                                            {this.state.shifts.map(shift => <MenuItem value={shift}>{shift}</MenuItem>)}
                                        </Select>
                                    </FormControl>
                                </div>
                                <div class="col-sm-12 col-md-4">
                                    <FormControl>
                                        ...
                                        >
                                            {this.state.versions.map(version => <MenuItem value={version}>{version}</MenuItem>)}
                                        </Select>
                                    </FormControl>
                                </div>
                                <div class="col-sm-12 col-md-4">
                                    <FormControl>
                                        ...
                                        >
                                            {this.state.groups.map(group => <MenuItem value={group}>{group}</MenuItem>)}
                                        </Select>
                                    </FormControl>
                                </div>
                            </div>
                    ...
                    <DialogActions>
                        ...
                        <Button form="update-svg-form" color="primary" onClick={this.addAdmissionData}>
                            Save
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    <DialogTitle style={{ cursor: 'move', textAlign: 'center' }} id="draggable-dialog-title">
                        Update Existing Admission Entry {this.state.versionName}{this.state.hscGroupName}
                    </DialogTitle>
                    <DialogContent>
                        <form id="upload-admission-data-form" >
                            <div class="row upload-admission-data-form">
                    ...
                    <DialogActions>
                        <Button autoFocus color="primary" onClick={() => this.setState({ showEditWindow: false })}>
                            Cancel
                        </Button>
                        <Button form="update-svg-form" color="primary" onClick={this.updateAdmissionData}>
                            Update
                        </Button>
                    </DialogActions>
                </Dialog>
