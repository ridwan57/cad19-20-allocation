...
import { FormControl, InputLabel, Select, MenuItem, Fab, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Button, IconButton, TextField, Checkbox } from '@material-ui/core';
...
import { Col, Row, Nav, Card, Image, Table, Dropdown, ProgressBar, Pagination, ButtonGroup, Accordion } from '@themesberg/react-bootstrap';
...
import { studentModuleRootAddress, boardModuleRootAddress } from '../../../data/constants';
...
class SubmitApplication extends Component {
    constructor(props) {
        ...
        this.handleAddChoiceButtonPressed = this.handleAddChoiceButtonPressed.bind(this);
        this.handlePriorityUpButtonPressed = this.handlePriorityUpButtonPressed.bind(this);
        this.handlePriorityDownButtonPressed = this.handlePriorityDownButtonPressed.bind(this);
        this.handleRemoveChoiceButtonPressed = this.handleRemoveChoiceButtonPressed.bind(this);
        this.handleOverflowAcceptedButtonPressed = this.handleOverflowAcceptedButtonPressed.bind(this);
        ...
        this.handleDraftChoiceExistAcceptedButtonPressed = this.handleDraftChoiceExistAcceptedButtonPressed.bind(this);
        this.handleUpdateButtonPressed = this.handleUpdateButtonPressed.bind(this);
        this.handleSubmitButtonPressed = this.handleSubmitButtonPressed.bind(this);
        this.submitApplicationSuccessMessageAcceptedButtonPressed = this.submitApplicationSuccessMessageAcceptedButtonPressed.bind(this);
    ...
    compareChoiceLists(choiceListA, choiceListB) {
        ...
        for (let index = 0; index < choiceListA.length; index++) {
            ...
            if (choiceA.priority !== choiceB.priority
    ...
    componentDidMount() {
            ...
            fetch(boardModuleRootAddress + '/getScheduleData/' + lastesPhase, {
                ...
                data.result.forEach(schedule => {
                    if (schedule.name === 'APPLICATION') {
                        ...
                        console.log(moment(schedule.startDateTime).format('YYYY-MM-DD HH:mm:ss'));
                        console.log(moment(schedule.endDateTime).format('YYYY-MM-DD HH:mm:ss'));
                        console.log(moment(schedule.startDateTime).format('YYYY-MM-DD HH:mm:ss') > moment().format('YYYY-MM-DD HH:mm:ss'));
                        if (moment(schedule.endDateTime).format('YYYY') !== '1900' && moment(schedule.endDateTime).format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')) {
                            console.log('Current Application phase is finished. Please wait until the next application phase opens');
                            this.setState({ notAllowedMessage: 'Current Application phase is finished. Please wait until the next application phase opens' })
                            this.setState({ applicationAllowed: false, notAllowedPrompt: true });
                        }
                        else if (moment(schedule.startDateTime).format('YYYY') !== '1900' && moment(schedule.startDateTime).format('YYYY-MM-DD HH:mm:ss') > moment().format('YYYY-MM-DD HH:mm:ss')) {
                            console.log('The next application phase is scheduled to start from ' + moment(schedule.startDateTime).format('YYYY-MM-DD hh:mm A') + '. Please try later');
                            this.setState({ notAllowedMessage: 'The next application phase is scheduled to start from ' + moment(schedule.startDateTime).format('YYYY-MM-DD hh:mm A') + '. Please try later' })
                            this.setState({ applicationAllowed: false, notAllowedPrompt: true });
                        }
                        else if (moment(schedule.endDateTime).format('YYYY') === '1900') {
                            console.log('Application Process has not been started yet. Please wait until the next application phase opens');
                            this.setState({ notAllowedMessage: 'Application Process has not been started yet. Please wait until the next application phase opens' })
                            this.setState({ applicationAllowed: false, notAllowedPrompt: true });
                        }
                    }
                });
        ...
        fetch(studentModuleRootAddress + '/getApplicationChoices', {
            ...
            .then(data => {
                if (data.result.length > 0) {
                    ...
                    this.id = setTimeout(() => this.setState({ draftChoicesExist: false }), 3000);
                }
            })
        ...
        if ((!this.state.singleColumn && window.innerWidth < 1800) || (this.state.singleColumn && window.innerWidth < 920)) this.setState({ shrink: true })
    ...
    changeQuotaSelection(index) {
        ...
        updatedQuotaStatus[index].checkbox = <Checkbox
            onChange={() => this.changeQuotaSelection(index)}
            color="primary"
            ...
            .then(data => {
                ...
                lst.forEach(svg => {
                    idx = this.state.choices.findIndex(function (choice) {
                        ...
                        return choice.eiin === svg.eiin && choice.shift.shiftId === svg.shift.shiftId
                            && choice.version.versionId === svg.version.versionId && choice.hscGroup.hscGroupId === svg.hscGroup.hscGroupId
                    });
                    if (idx === -1) // svg is not exist in choice list
                    {
                    ...
                    if (this.isSVGAllowed(svg)) {
                        ...
                        index += 1;
                        svg.gender = (svg.gender === 'F' && 'Female') || (svg.gender === 'M' && 'Male') || (svg.gender === 'C' && 'Co-Ed.');
                    }
                });
    ...
    isSVGAllowed(svg) {
        ...
        if (idx === -1) // this hsc group is not allowed by group mapping
        {
            { console.log("0"); return false; }
        }
        if (this.state.sscEIIN === svg.eiin && this.state.sscGPA < svg.ownMinimumGPA) // he is a student of this eiin and his gpa is less than allowed own gpa
        { console.log("1"); return false; }
        if (this.state.sscEIIN !== svg.eiin && this.state.sscGPA < svg.minimumGPA) // he is a student of this eiin and his gpa is less than allowed own gpa
        { console.log("2"); return false; }
        if ((this.state.gender.charAt(0) === 'M' && svg.gender.charAt(0) === 'F') || (this.state.gender.charAt(0) === 'F' && svg.gender.charAt(0) === 'M')) { console.log("3"); return false; }
        return isAllowed;
    }
    ...
    handleRemoveChoiceButtonPressed(choice) {
        ...
        if (isEIINtoDelelete === - 1) {
            this.state.selectedEIINList.splice(this.state.selectedEIINList.indexOf(choice.eiin));
            this.setState({ selectedEIINCount: this.state.selectedEIINCount - 1 });
        }
        ...
        const indexOfRemovedESVG = this.state.esvgs.findIndex(function (svg) {
            return (displayingEIIN === choice.eiin) && choice.shift.shiftId === svg.shift.shiftId
                && choice.version.versionId === svg.version.versionId && choice.hscGroup.hscGroupId === svg.hscGroup.hscGroupId
        });
    ...
    render() {
        ...
        return (
            (this.state.redirect && <Redirect to={Routes.studentUser.Application.ViewApplication.path} />)
            ||
            (this.state.applicationSubmittedAck && <Redirect to={Routes.studentUser.Application.ViewApplication.path} />)
            ||
            <>
                ...
                >
                    ...
                    <DialogContent>
                        <DialogContentText className="dialoge-text">
                            You already have a draft Application Choices saved. You can update it at your preference.
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions className="dialoge-text">
                        <Button onClick={this.handleDraftChoiceExistAcceptedButtonPressed} color="primary" autoFocus>
                            OK
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    ...
                    <DialogActions className="dialoge-text">
                        <Button onClick={() => this.setState({ showQuotaUpdateSuccessfull: false })} color="primary" autoFocus >
                            OK
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                <Dialog
                    ...
                    aria-describedby="alert-dialog-description"
                >
                    <DialogTitle className="dialoge-text">{"Can Not Add New EIIN"}</DialogTitle>
                    <DialogContent>
                        <DialogContentText className="dialoge-text">
                            You have already added the maximum number of distinct EIIN (10) to your choice list.
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions className="dialoge-text">
                        <Button onClick={this.handleOverflowAcceptedButtonPressed} color="primary" autoFocus>
                            OK
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                <Dialog
                    ...
                    aria-describedby="alert-dialog-description"
                >
                    <DialogTitle className="dialoge-text">{"Successfully Saved"}</DialogTitle>
                    <DialogContent>
                        <DialogContentText className="dialoge-text">
                            You have successfully saved your draft choice list. We are redirecting you to View Application Page.
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions className="dialoge-text">
                        <Button onClick={this.handleSaveSuccessfullAcceptedButtonPressed} color="primary" autoFocus>
                            OK
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    ...
                    <DialogContent>
                        <DialogContentText className="dialoge-text">
                            You have successfully submitted your College Application Choices. Press OK to view your submitted Application Choices.
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions className="dialoge-text">
                        <Button onClick={this.handleSaveSuccessfullAcceptedButtonPressed} color="primary" autoFocus>
                                    ...
                                    <div class="col-xl-12 col-40-pct">
                                        <Accordion defaultActiveKey="1" className="quota-selection" >
                                            <Accordion.Item eventKey="1">
                                                <Accordion.Header>
                                                    <h5 style={{ paddingLeft: '4px' }}>Quota Selection</h5>
                                                </Accordion.Header>
                                                <Accordion.Body>
                                                    <div className="row">
                                                        {
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-12" style={{ paddingTop: "20px", textAlign: "right" }}>
                                                            <Button
                                        ...
                                        <div class="col d-flex justify-content-center higher-padded-card" style={{ paddingTop: "20px" }}>
                                        ...
                                        */}
                                        <div class="col d-flex justify-content-center lower-padded-card">
                                            <Card style={{ width: "100%", alignContent: "center", display: "flex", position: "center", border: "1" }}>
                                                <Card.Header style={{ minHeight: "70px", paddingBottom: "-20px" }}>
                                                    {
                                                        this.state.college !== '' &&
                                                        <>
                                                            <h6 style={{ display: 'inline' }}>Available ESVG list of </h6>
                                                            <h5 style={{ display: 'inline' }}>{this.state.college + ' - ' + this.state.collegeName}</h5>
                                                        </>
                                                    }
                                                </Card.Header>
                                                <Card.Body>
                                                    <Table responsive className="table-centered table-nowrap rounded mb-0 choice-table">
                                                                    ...
                                                                    (
                                                                        (esvg, id) =>
                                                                            <tr>
                                                                                ...
                                                                                <td>
                                                                                    <Fab aria-label="add" color={(!esvg.alreadyExistInChoiceList && "primary") || "default"} style={{ marginTop: "-4px", width: "32px", height: "32px", minHeight: "32px" }} onClick={() => !esvg.alreadyExistInChoiceList && this.handleAddChoiceButtonPressed(esvg)}>
                                                                                        <AddIcon style={{ width: "0.88em", height: "0.88em" }} />
                                                                                    </Fab>
                                                                                </td>
                                                                            </tr>
                                                                    )
                                        ...
                                        <div style={{ width: "100%" }}>
                                            <div class="col d-flex justify-content-center view-application">
                                                <Accordion defaultActiveKey="1" className="quota-selection" >
                                                    <Accordion.Item eventKey="1">
                                                        <Accordion.Header>
                                                            <h6 style={{ display: "inline" }}>Choice Summary of </h6><h5 style={{ paddingLeft: '4px', display: "inline" }}>{this.props.keycloak.tokenParsed.name}</h5>
                                                        </Accordion.Header>
                                                        <Accordion.Body>
                                                            <div class="row">
                                                                <div class="col-sm-6 col-md-6 col-33-pct">
                                                                    <h7>Minimum EIIN Required: {this.state.minimumEIINCount}</h7>
                                                                </div>

                                                                <div class="col-sm-6 col-md-6 col-33-pct">
                                                                    <h7>Maximum EIIN Allowed: {this.state.maximumEIINCount}</h7>
                                                                </div>

                                                                <div class="col-sm-6 col-md-6 col-33-pct">
                                                                    <h7>Selected EIIN Count: {this.state.selectedEIINCount}</h7>
                                                                </div>
                                                            </div>
                                                        </Accordion.Body>
                                                    </Accordion.Item>
                                            ...
                                            <div class="col d-flex justify-content-center lower-padded-card">
                                                <Card style={{ width: "100%", alignContent: "center", position: "center", border: "1" }}>
                                                    <Card.Header>
                                                        <h6 style={{ display: "inline" }}>Choice List of </h6>
                                                        <h5 style={{ display: "inline" }}><b>{this.props.keycloak.tokenParsed.name}</b></h5>
                                                    </Card.Header>
                                                    <Card.Body>
                                                        <Table responsive className="table-centered table-wrap rounded mb-0 choice-table">
                                                            <thead className="thead-light">
                                                                <tr>
                                                                        ...
                                                                        (
                                                                            (choice, id) =>
                                                                                <tr>
                                                                                    ...
                                                                                    <td>
                                                                                        <Fab color={((choice.priority) === 1 && "default") || "primary"} aria-label="add" style={{ marginTop: "-4px", width: "32px", height: "32px", minHeight: "32px" }} onClick={() => (choice.priority) !== 1 && this.handlePriorityUpButtonPressed(choice)}>
                                                                                            <ArrowUpwardIcon style={{ width: "0.88em", height: "0.88em" }} />
                                                                                        </Fab>
                                                                                        &nbsp;
                                                                                        <Fab color={((choice.priority) === this.state.lastPriority && "default") || "primary"} aria-label="add" style={{ marginTop: "-4px", width: "32px", height: "32px", minHeight: "32px" }} onClick={() => (choice.priority) !== this.state.lastPriority && this.handlePriorityDownButtonPressed(choice)}>
                                                                                            <ArrowDownwardIcon style={{ width: "0.88em", height: "0.88em" }} />
                                                                                        </Fab>
                                                                                        &nbsp;
                                                                                        <Fab color="secondary" aria-label="add" style={{ marginTop: "-4px", width: "32px", height: "32px", minHeight: "32px" }} onClick={() => this.handleRemoveChoiceButtonPressed(choice)}>
                                                                                            <ClearIcon style={{ width: "0.88em", height: "0.88em" }} />
                                                                                        </Fab>
                                                                                    </td>
                                                                                </tr>
                                                                        )
                                            ...
                                            <div class="row">
                                                <div class="col-xm-12 col-md-6" style={{ paddingTop: "20px", textAlign: "center" }}>
                                                    <Button
                                                        onClick={(this.state.showSaveButton && this.handleSaveButtonPressed) || (this.state.showUpdateButton && this.handleUpdateButtonPressed)}
                                                        variant="contained"
                                                        color="primary"
                                                        size="medium"
                                                        disabled={(this.state.lastPriority === 0 || !this.state.draftChoicesUpdated)}
                                                        startIcon={<SaveIcon />}
                                                    >
                                                        {this.state.showSaveButton && "Save Application"}
                                                        {this.state.showUpdateButton && "Update Application"}
                                                    </Button>
                                                </div>
                                                <div class="col-xm-12 col-md-6" style={{ paddingTop: "20px", textAlign: "center" }}>
                                                    <Button
                                                        ...
                                                        color="primary"
                                                        size="medium"
                                                        disabled={this.state.draftChoicesUpdated || !this.state.choices.length}
                                                        startIcon={<SaveIcon />}
                                                    >
                                                        Submit Application
                                                    </Button>
