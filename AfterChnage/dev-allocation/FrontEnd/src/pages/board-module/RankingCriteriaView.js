...
import { boardModuleRootAddress, rankingTypeCodes, rankingOrderTextualFields, rankingTypeTextToCode, LROCodes, rankingCategory } from '../../data/constants';
...
class RankingCriteriaView extends Component {
            ...
            .then(data => {
                if (data.result) {
                    ...
                    rankingCriteria.forEach(element => {
                        element.sortingOrderTextual = rankingOrderTextualFields[element.sortingOrder];
                        element.rankingSubCriteria.sort(function (a, b) {
                            if (a.priority < b.priority) return -1;
                            if (a.priority > b.priority) return 1;
                            return 0;
                        });
                        element.subCriteriaFields = element.rankingSubCriteria.map(subElement => <div class="box">{subElement.displayName}</div>)
                        if (criteriaCode === rankingTypeCodes.scienceTypeRankingCode && element.priorityScience > 0) {
                            element.priority = element.priorityScience;
                            scienceCriteria.push(element);
                        }
                        else if (criteriaCode === rankingTypeCodes.allTypeRankingCode && element.priorityAll > 0) {
            ...
            fetch(boardModuleRootAddress + '/getScheduleData/0', {
                ...
                data.result.forEach(schedule => {
                    if (schedule.name === 'LOCK_SSC_RESULT') {
                        ...
                        if (moment(schedule.startDateTime).format('YYYY') !== '1900' && moment(schedule.startDateTime).format('YYYY-MM-DD HH:mm:ss') > moment().format('YYYY-MM-DD HH:mm:ss')) {
                            console.log('SSC result date is yet to be locked. Pre Ranking is not allowed');
                            this.setState({ notAllowedMessage: 'SSC result date is yet to be locked. Pre Ranking is not allowed' })
                    ...
                    data.result.forEach(schedule => {
                        if (schedule.name === 'APPLICATION') {
                            ...
                            if (moment(schedule.endDateTime).format('YYYY') !== '1900' && moment(schedule.endDateTime).format('YYYY-MM-DD HH:mm:ss') > moment().format('YYYY-MM-DD HH:mm:ss')) {
                                console.log('Current Application phase is yet to be finished. Post Ranking is not allowed');
                                this.setState({ notAllowedMessage: 'Current Application phase is yet to be finished. Post Ranking is not allowed' })
                                this.setState({ rankingAllowed: false });
                            }
                        }
                    });
    ...
    render() {
        ...
        if (this.state.moveToLanding) this.props.updateStateFunction('landing', this.props.rankingType);
        return (
            <>
                ...
                >
                    ...
                    <DialogActions style={{ justifyContent: 'center', marginBottom: '20px' }}>
                        <Button onClick={() => this.setState({ notAllowedPrompt: false })} color="primary" variant="contained" autoFocus>
                            OK
                        </Button>
                    </DialogActions>
                </Dialog>
                ...
                >
                    ...
                    <DialogActions style={{ justifyContent: 'center', marginBottom: '20px'}}>
                        <Button onClick={this.cancelRanking} color="secondary" variant="contained" autoFocus>
                            Cancel
                        </Button>
                        <Button onClick={this.startRanking} color="primary" variant="contained" autoFocus>
                            Confirm
                        </Button>
                    </DialogActions>
                </Dialog>
                                ...
                                <Card>
                                    <Card.Header>
                                        <div className="row">
                                            <div className="col-6">
                                                <Tooltip title="Back">
                                                    <IconButton color="black" onClick={() => this.props.updateStateFunction('landing', this.props.rankingType)}>
                                                        <ArrowBackIcon />
                                                    </IconButton>
                                                </Tooltip>
                                            ...
                                            <div className='col-6 button-div'>
                                                <Button
                                                    ...
                                                    onClick={() => this.props.updateStateFunction('criteria-edit', this.props.rankingType)}
                                                    variant="contained"
                                                    color="primary"
                                                    size="medium"
                                                    disabled={false}
