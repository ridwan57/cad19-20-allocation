...
// .css-grh1if-MuiButtonBase-root-MuiFab-root.Mui-disabled
...
class ManageSQApplications extends Component {
  constructor (props) {
    ...
    this.handleViewDetailsButtonPressed = this.handleViewDetailsButtonPressed.bind(
      this
    )
    this.handleGrantedButtonPressed = this.handleGrantedButtonPressed.bind(this)
    this.handleRejectedButtonPressed = this.handleRejectedButtonPressed.bind(
      this
    )
  ...
  fetchSQApplicantions () {
    fetch(institutionModuleRootAddress + '/getSpecialQuotaRequestList/', {
      ...
      .then(data => {
        ...
        var lst = []
        if (data.status === 200) {
          data.result.map(application => lst.push(application))
          const actionButton = (
            <Button onClick={this.handleClick}>Action</Button>
          )
          lst.forEach((element, index) => {
            element.action = (
              <Button onClick={() => this.handleClick(index)}>Action</Button>
            )
            element.id = index + 1
              ...
              <div>
                ...
                <Tooltip title='Grant'>
                  <IconButton
                    ...
                    size='small'
                    disabled={element.sqStatus !== 'Applied'}
                    className={{}}
                  >
                    <DoneOutlineRoundedIcon
                      ...
                      sx={{
                        color: `${
                          element.sqStatus === 'Applied'
                            ? 'rgba(14, 134, 37, 1)'
                            : 'rgba(14, 134, 37,.2)'
                        }`
                      }}
                      onClick={() => this.handleGrantedButtonPressed(index)}
                ...
                <Tooltip title='Reject'>
                  ...
                  >
                    <CancelIcon
                      ...
                      onClick={() => this.handleRejectedButtonPressed(index)}
      ...
      <>
        ...
        >
          ...
          <DialogContent>
            ...
            >
              You have successfully {this.state.updateStatus} the SQ
              Applicantion of{' '}
              {this.state.updatedApplicant &&
                this.state.updatedApplicant.studentName}{' '}
              (CRVSID:{' '}
              {this.state.updatedApplicant &&
                this.state.updatedApplicant.crvsId}
              ).
            </DialogContentText>
          </DialogContent>
          <DialogActions
          ...
          >
            <Button
              onClick={this.SQStatusUpdateSuccessAck}
              color='primary'
              variant='contained'
              autoFocus
            >
