...
import { boardModuleRootAddress, defaultPhaseData, defaultScheduleData, scheduleStatuses } from '../../data/constants';
...
export default function VerticalLinearStepper(props) {
    ...
    const savePhaseData = () => {
        ...
        initialPhaseData.forEach(schedule => {
            schedule.startDateTime = moment(schedule.date).format('yyyy-MM-DD') + ' ' + moment(schedule.time).format('HH:mm:ss');
        ...
        phaseTaskData.forEach(phase => {
            phase.forEach(taskData => {
                if (taskData.sameStartEnd) {
                    taskData.startDateTime = moment(taskData.startDate).format('yyyy-MM-DD') + ' ' + moment(taskData.startTime).format('HH:mm:ss');
                    taskData.endDateTime = taskData.startDateTime;
                    scheduleArrayToSubmit.push(taskData);
                }
                else {
                    taskData.startDateTime = moment(taskData.startDate).format('yyyy-MM-DD') + ' ' + moment(taskData.startTime).format('HH:mm:ss');
                    taskData.endDateTime = moment(taskData.endDate).format('yyyy-MM-DD') + ' ' + moment(taskData.endTime).format('HH:mm:ss');
                    scheduleArrayToSubmit.push(taskData);
                }
            })
        })
    ...
    function getSteps() {
        return ['Time Table for Data Synchronization', 'Phases of Application Processing'];
    }
    ...
    const updatePhaseTaskData = (dateTime, phaseIndex, indexPhaseData, key1, key2) => {
            ...
            fetch(boardModuleRootAddress + '/getPhaseCount', {
                ...
                for (let phase = 1; phase < parseInt(data.result); phase++) {
                    fetch(boardModuleRootAddress + '/getScheduleData/' + phase, {
                        ...
                        //console.log(data.result);
                        data.result.forEach(schedule => {
                            if (schedule.startDateTime === schedule.endDateTime) schedule.sameStartEnd = true;
                            if (moment(schedule.startDateTime).format('YYYY') === '1900') {
    ...
    function getStepContent(step) {
        switch (step) {
                                        ...
                                        <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                            <Grid container justifycontent="space-around">
                                                <KeyboardDatePicker
                                                    ...
                                                    value={schedule['date']}
                                                    onChange={(date) => updateInitialPhaseData(date, index, 'date')}
                                                    KeyboardButtonProps={{
                                                        'aria-label': 'change date',
                                                    }}
                                                    disabled={scheduleStatus === scheduleStatuses.validState}
                                                //autoOk={true}
                                                />
                                            </Grid>
                                        </MuiPickersUtilsProvider>
                                        ...
                                        <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                            <Grid container justifycontent="space-around">
                                                <KeyboardTimePicker
                                                    ...
                                                    label="Time"
                                                    value={schedule['time']}
                                                    onChange={(time) => updateInitialPhaseData(time, index, 'time')}
                                                    KeyboardButtonProps={{
            ...
            case 1:
                return (
                    phaseTaskData.map((phase, phaseIndex) =>
                        <div>
                            <div className="bordered-div">
                                <Accordion defaultExpanded={true}>
                                    <AccordionSummary
                                        ...
                                        id="panel1a-header"
                                    >
                                        <Typography className={classes.heading}><b>Phase {phaseIndex + 1}</b></Typography>
                                    </AccordionSummary>
                                    <AccordionDetails>
                                                                            ...
                                                                            <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                                                                <Grid container justifycontent="space-around">
                                                                                    <KeyboardDatePicker
                                                                                        ...
                                                                                        onChange={(date) => updatePhaseTaskData(date, phaseIndex, indexPhaseData, 'startDate', null)}
                                                                                        KeyboardButtonProps={{
                                                                                            'aria-label': 'change date',
                                                                                        }}
                                                                                        disabled={scheduleStatus === scheduleStatuses.validState}
                                                                            ...
                                                                            <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                                                                <Grid container justifycontent="space-around">
                                                                                    <KeyboardTimePicker
                                                                                        ...
                                                                                        onChange={(time) => updatePhaseTaskData(time, phaseIndex, indexPhaseData, 'startTime', null)}
                                                                                        KeyboardButtonProps={{
                                                                                            'aria-label': 'change time',
                                                                                        }}
                                                                                        keyboardIcon={<AccessTimeIcon />}
                                                                            ...
                                                                            <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                                                                <Grid container justifycontent="space-around">
                                                                                    <KeyboardDatePicker
                                                                                        ...
                                                                                        onChange={(date) => updatePhaseTaskData(date, phaseIndex, indexPhaseData, 'endDate', null)}
                                                                                        KeyboardButtonProps={{
                                                                                            'aria-label': 'change date',
                                                                                        }}
                                                                                        disabled={scheduleStatus === scheduleStatuses.validState}
                                                                            ...
                                                                            <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                                                                <Grid container justifycontent="space-around">
                                                                                    <KeyboardTimePicker
                                                                                        ...
                                                                                        onChange={(time) => updatePhaseTaskData(time, phaseIndex, indexPhaseData, 'endTime', null)}
                                                                                        KeyboardButtonProps={{
                                                                                            'aria-label': 'change time',
                                                                                        }}
                                                                                        keyboardIcon={<AccessTimeIcon />}
                                                                    ...
                                                                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                                                        <Grid container justifycontent="space-around">
                                                                            <KeyboardDatePicker
                                                                                ...
                                                                                placeholder={"Enter a Date"}
                                                                                value={phaseData['startDate']}
                                                                                onChange={(date) => updatePhaseTaskData(date, phaseIndex, indexPhaseData, 'startDate', 'endDate')}
                                                                                KeyboardButtonProps={{
                                                                                    'aria-label': 'change date',
                                                                                }}
                                                                                disabled={scheduleStatus === scheduleStatuses.validState}
                                                                    ...
                                                                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                                                        <Grid container justifycontent="space-around">
                                                                            <KeyboardTimePicker
                                                                                ...
                                                                                label="Time"
                                                                                value={phaseData['startTime']}
                                                                                onChange={(time) => updatePhaseTaskData(time, phaseIndex, indexPhaseData, 'startTime', 'endTime')}
                                                                                KeyboardButtonProps={{
                                                                                    'aria-label': 'change time',
                                                                                }}
                                                                                keyboardIcon={<AccessTimeIcon />}
    ...
    if (scheduleStatus === scheduleStatuses.createNewSchedule || scheduleStatus === scheduleStatuses.validState || scheduleStatus === scheduleStatuses.editState) {
        return (
                ...
                >
                    ...
                    <DialogContent>
                        <DialogContentText className="dialoge-text">
                            Do your really want to {scheduleStatus === scheduleStatuses.editState ? 'update' : 'save'} the schedule data?
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions className="dialoge-text" style={{ justifyContent: 'center', marginBottom: '20px' }}>
                        <Button onClick={() => setSavePrompt(false)} color="secondary" variant='contained' autoFocus>
                            No
                        </Button>
                        <Button onClick={savePhaseData} color="primary" variant='contained' autoFocus>
                            Yes
                        </Button>
                    </DialogActions>
                </Dialog>
                    ...
                    <div className="d-block mb-4 mb-xl-0">
                        <Breadcrumb className="d-none d-md-inline-block" listProps={{ className: "breadcrumb-dark breadcrumb-transparent" }}>
                            ...
                            <Breadcrumb.Item active>
                                {scheduleStatus === scheduleStatuses.createNewSchedule && 'Create Schedule'}
                                {scheduleStatus === scheduleStatuses.validState && 'Scheduling Details'}
                                {scheduleStatus === scheduleStatuses.editState && 'Edit Schedule'}
                            </Breadcrumb.Item>
                        </Breadcrumb>
                                ...
                                <Card.Header>
                                    <div className='row'>
                                        ...
                                        </div>
                                        {
                                            ...
                                            <div className='col-6' style={{ textAlign: 'right' }}>
                                                <Button
                                                    onClick={() => setScheduleStatus(scheduleStatuses.editState)}
                                                    ...
                                                    <StepContent>
                                                        ...
                                                        <div className={classes.actionsContainer}>
                                                            ...
                                                            <div>
                                                                ...
                                                                {
                                                                    ((scheduleStatus === scheduleStatuses.createNewSchedule || scheduleStatus === scheduleStatuses.editState) && activeStep === steps.length - 1) &&
                                                                    <Button
                                                                        variant="contained"
                                                                        color="primary"
                                                                        onClick={handleNext}
                                                                ...
                                                                {
                                                                    (scheduleStatus === scheduleStatuses.createNewSchedule || scheduleStatus === scheduleStatuses.editState) &&
                                                                    <Button
                                                                        variant="contained"
                                                                        color="secondary"
                                                                        onClick={() => {
                                                                            if (scheduleStatus === scheduleStatuses.createNewSchedule) {
                                                                                setScheduleStatus(scheduleStatuses.resetState);
                                                                                handleReset();
                                                                            }
                                                                            else {
                                            ...
                                            <Paper square elevation={0} className={classes.resetContainer}>
                                                ...
                                                </Button>
                                                {
                                                    ...
                                                    <Button
                                                        ...
                                                        color="primary"
                                                    >
                                                        {scheduleStatus === scheduleStatuses.editState ? 'Update' : 'Save'}
                                                    </Button>
                                                }
